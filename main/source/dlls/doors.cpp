...
void CBaseDoor::ResetEntity()
{
	this->SetToggleState(TS_AT_BOTTOM);
}
...
void CBaseDoor :: SetToggleState( int state )
{
	...
	this->m_toggle_state = (TOGGLE_STATE)state;
}
...
void CBaseDoor::DoorHitTop( void )
{
	...
	// This line is crashing...fix it?  I think it's because doors are being manually reset
...
//		Valve fix for super-loud doors!
//		if (m_flWait >= 0)
//		{
//		if (m_toggle_state == TS_GOING_DOWN)
	...
	if(!AvHSUGetIsExternalClassName(STRING(pOther->pev->classname)))
	{
		AvHDeployedMine* theMine = dynamic_cast<AvHDeployedMine*>(pOther);
		if(theMine)
		{
			theMine->Detonate();
		}
				...
				if ( FClassnameIs ( pentTarget, "func_door" ) || FClassnameIs ( pentTarget, "func_door_rotating" ) )
				{
					...
					if ( pDoor->m_flWait >= 0)
					{
						if (pDoor->pev->velocity == pev->velocity && pDoor->pev->avelocity == pev->velocity)
						{
							...
							if ( FClassnameIs ( pentTarget, kesFuncDoor ) )
