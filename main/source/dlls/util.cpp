...
int UTIL_CountEntitiesInSphere(const Vector& inVecCenter, float inRadius, const char* inClassName)
{
	int theNumEntities = 0;

	CBaseEntity* theEntity = NULL;
	while((theEntity = UTIL_FindEntityInSphere(theEntity, inVecCenter, inRadius)) != NULL)
	{
		if(!strcmp(STRING(theEntity->pev->classname), inClassName))
		{
			theNumEntities++;
		}
	}
...
void UTIL_SayText( const char *pText, CBaseEntity *pEntity, int inEntIndex)
{
	...
	if(inEntIndex == -1)
	{
		inEntIndex = pEntity->entindex();
	}
	else
	{
		CBaseEntity* theSpeakingPlayer = CBaseEntity::Instance(g_engfuncs.pfnPEntityOfEntIndex(inEntIndex));
		if(theSpeakingPlayer)
		{
			theLocation = AvHSUGetLocationText(theSpeakingPlayer);
		}
	}
...
void UTIL_Tutorial( CBaseEntity *pEntity, const char *pMessage )
{
	...
	theTextParms.b2 = 240;
	theTextParms.a2 = 128;
	theTextParms.fadeinTime = .3f;
	theTextParms.fadeoutTime = .3f;
	theTextParms.holdTime = 6.0f;
	theTextParms.fxTime = .25f;
	theTextParms.channel = 1;

	UTIL_HudMessage( pEntity, theTextParms, pMessage);
}

void UTIL_Error( CBaseEntity *pEntity, const char *pMessage )
{
	hudtextparms_t	theTextParms;
