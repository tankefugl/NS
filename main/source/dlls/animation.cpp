...
const char* LookupSequence( void *pmodel, int inSequence)
{
	const char* theSequenceName = NULL;
	...
	if (pstudiohdr)
	{
		...
		if((inSequence >= 0) && (inSequence < pstudiohdr->numseq))
		{
			theSequenceName = pseqdesc[inSequence].label;
		}
	}
...
{
	float theSequenceDuration = 0.0f;
	...
	if(pstudiohdr)
	{
		...
		if(pseqdesc->fps > 0)
		{
			theSequenceDuration = (pseqdesc->numframes - 1)/pseqdesc->fps;
		}
	}
	
	return theSequenceDuration;
}
