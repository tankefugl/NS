...
char* CBaseTurret::GetPingSound() const
{
	return "turret/tu_ping.wav";
}

char* CBaseTurret::GetActiveSound() const
{
	return "turret/tu_active2.wav";
}

char* CBaseTurret::GetAlertSound() const
{
	return "turret/tu_alert.wav";
}

char* CBaseTurret::GetDeploySound() const
{
...
void CBaseTurret::Ping( void )
{
	...
	if (m_flPingTime == 0)
		m_flPingTime = gpGlobals->time + kPingInterval;
	else if (m_flPingTime <= gpGlobals->time)
	{
		m_flPingTime = gpGlobals->time + kPingInterval;
		EMIT_SOUND(ENT(pev), CHAN_ITEM, this->GetPingSound(), kPingVolume, ATTN_STATIC);
		EyeOn( );
	}
...
bool CBaseTurret::NeedsLineOfSight() const
{
	return true;
}
...
{
	FireBullets( 1, vecSrc, vecDirToEnemy, TURRET_SPREAD, this->GetRange(), BULLET_MONSTER_12MM, 1 );
...
{
	FireBullets( 1, vecSrc, vecDirToEnemy, TURRET_SPREAD, this->GetRange(), BULLET_MONSTER_9MM, 1 );
...
void CBaseTurret::Deploy(void)
{
	...
	if (pev->sequence != TURRET_ANIM_DEPLOY)
	{
		...
		EMIT_SOUND(ENT(pev), CHAN_BODY, this->GetDeploySound(), TURRET_MACHINE_VOLUME, ATTN_NORM);
...
int CBaseTurret::GetRange() const
{
	return TURRET_RANGE;
}
...
void CBaseTurret::AutoSearchThink(void)
{
	...
	{
		SetThink(&CBaseTurret::Deploy);
		EMIT_SOUND(ENT(pev), CHAN_BODY, this->GetAlertSound(), TURRET_MACHINE_VOLUME, ATTN_NORM);
	}
}
...
void CBaseTurret ::	TurretDeath( void )
{
	...
	if (pev->dmgtime + RANDOM_FLOAT( 0, 5 ) > gpGlobals->time)
	{
		...
		if (m_iOrientation == 0)
			...
			vecSrc = vecSrc + Vector( (float)0, (float)0, (float)RANDOM_FLOAT( pev->absmin.z, pev->origin.z ) );
...
{
	CBaseTurret::Precache( );
	PRECACHE_MODEL (kDeployedTurretModel);	
}
...
{
	FireBullets( 1, vecSrc, vecDirToEnemy, TURRET_SPREAD, this->GetRange(), BULLET_MONSTER_MP5, 1 );
