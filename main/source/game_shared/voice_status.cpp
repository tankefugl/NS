...
void CVoiceStatus::CreateEntities()
{
	...
	for(int i=0; i < VOICE_MAX_PLAYERS; i++)
	{
		...
		pEnt->origin[2] = AvHCUGetIconHeightForPlayer((AvHUser3)pClient->curstate.iuser3);
		...
		if(bTalking)
		{
			...
			if(!pLabel)
			{
				if(pLabel = GetFreeVoiceLabel())
				{
					...
					if( pLabel->m_pLabel )
					{
						pLabel->m_pLabel->setFgColor( 0, 0, 0, 0 );
...
void CVoiceStatus::RepositionLabels()
{
	...
	if( m_pLocalBitmap && m_pAckBitmap && m_pLocalLabel && (m_bTalking || m_bServerAcked) )
	{
		...
		case AVH_USER3_ALIEN_PLAYER5:
			local_xPos -= kResourceEnergyBarWidth*ScreenWidth();
			break;

		case AVH_USER3_COMMANDER_PLAYER:
			local_xPos -= kVoiceStatusCommanderRightEdgeInset*ScreenWidth();
			local_yPos -= (1.0f - kVoiceStatusCommanderTopEdgeInset)*ScreenHeight();
			break;

		case AVH_USER3_MARINE_PLAYER:
			local_xPos -= kVoiceStatusMarineRightEdgeInset*ScreenWidth();
			break;
		}
